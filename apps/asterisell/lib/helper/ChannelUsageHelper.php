...
class StatsOnCalls
{
    ...
    public function __construct($condition, $startFilterDate, $endFilterDate = NULL)
    {
        ...
        $callDateField = 0;
        $durationField = 1;
        $destinationTypeField = 2;
        $idField = 3;
        $c->addAscendingOrderByColumn(ArCdrPeer::CALLDATE);
        ...
        //
        $this->totCalls = 0;
        $stmt = ArCdrPeer::doSelectStmt($c);
        $this->maxNrOfConcurrentCalls = 0;
        while ($rs = $stmt->fetchColumn()) {
            $callDateStamp = $rs[$callDateField];
            $callDate = strtotime($callDateStamp);
            $duration = $rs[$durationField];
            $destinationType = $rs[$destinationTypeField];
            $id = $rs[$idField];

            // Skip unprocessed/ignored calls
            //
            if ($destinationType == DestinationType::ignored) {
                continue;
            }
            $this->totCalls++;
