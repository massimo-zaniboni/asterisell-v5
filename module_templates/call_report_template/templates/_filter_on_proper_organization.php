...
if (isset($filters['filter_on_voip_account'])) {
    $selectedExtensionId = $filters['filter_on_voip_account'];
...
$options = OrganizationUnitInfo::getInstance()->getUIOrganizationSelector($allowedOrganizationId, true, false, false, false, ! $user->isAdmin());
if (! is_null($selectedOrganizationId)) {
    $defaultChoice = $selectedOrganizationId;
} else {
    $defaultChoice = "";
}

echo select_tag('filters[filter_on_proper_organization]', options_for_select($options, $defaultChoice));
