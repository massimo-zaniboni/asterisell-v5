...
class sfInflector
{
  ...
  public static function camelize($lower_case_and_underscored_word)
  {
      ...
      $text = preg_replace_callback('#/(.?)#', function ($matches) { return '::'.strtoupper($matches[0]); }, $lower_case_and_underscored_word);
      return preg_replace_callback('/(^|_|\.)+(.)/', function ($match) { return ('.' === $match[1] ? '_' : '').strtoupper($match[2]); }, $text);

    // Massimo Zaniboni: modified for supporting callback
    //
