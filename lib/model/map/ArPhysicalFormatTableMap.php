<?php


/**
 * This class defines the structure of the 'ar_physical_format' table.
 *
 *
 *
 * This map class is used by Propel to do runtime db structure discovery.
 * For example, the createSelectSql() method checks the type of a given column used in an
 * ORDER BY clause to know whether it needs to apply SQL to make the ORDER BY case-insensitive
 * (i.e. if it's a text column type).
 *
 * @package    lib.model.map
 */
class ArPhysicalFormatTableMap extends TableMap {

	/**
	 * The (dot-path) name of this class
	 */
	const CLASS_NAME = 'lib.model.map.ArPhysicalFormatTableMap';

	/**
	 * Initialize the table attributes, columns and validators
	 * Relations are not initialized by this method since they are lazy loaded
	 *
	 * @return     void
	 * @throws     PropelException
	 */
	public function initialize()
	{
	  // attributes
		$this->setName('ar_physical_format');
		$this->setPhpName('ArPhysicalFormat');
		$this->setClassname('ArPhysicalFormat');
		$this->setPackage('lib.model');
		$this->setUseIdGenerator(true);
		// columns
		$this->addPrimaryKey('ID', 'Id', 'INTEGER', true, null, null);
		$this->addForeignKey('AR_LOGICAL_SOURCE_ID', 'ArLogicalSourceId', 'INTEGER', 'ar_logical_source', 'ID', false, null, null);
		$this->addColumn('NAME', 'Name', 'VARCHAR', false, 200, null);
		$this->addColumn('DESCRIPTION', 'Description', 'VARCHAR', false, 2048, null);
		// validators
	} // initialize()

	/**
	 * Build the RelationMap objects for this table relationships
	 */
	public function buildRelations()
	{
    $this->addRelation('ArLogicalSource', 'ArLogicalSource', RelationMap::MANY_TO_ONE, array('ar_logical_source_id' => 'id', ), null, null);
    $this->addRelation('ArSourceCsvFile', 'ArSourceCsvFile', RelationMap::ONE_TO_MANY, array('id' => 'ar_physical_format_id', ), null, null);
    $this->addRelation('ArSourceCdr', 'ArSourceCdr', RelationMap::ONE_TO_MANY, array('id' => 'ar_physical_format_id', ), null, null);
	} // buildRelations()

	/**
	 * 
	 * Gets the list of behaviors registered for this table
	 * 
	 * @return array Associative array (name => parameters) of behaviors
	 */
	public function getBehaviors()
	{
		return array(
			'symfony' => array('form' => 'true', 'filter' => 'true', ),
		);
	} // getBehaviors()

} // ArPhysicalFormatTableMap
