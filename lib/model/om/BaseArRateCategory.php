...
/**
 * Base class that represents a row from the 'ar_rate_category' table.
 *
 * 
 *
...
abstract class BaseArRateCategory extends BaseObject  implements Persistent {
	 ...
	 * @return     ArRateCategory The current object (for fluent API support)
	 */
	...
	public function setInternalName($v)
	{
		...
		if ($this->internal_name !== $v) {
			...
			$this->modifiedColumns[] = ArRateCategoryPeer::INTERNAL_NAME;
		}
	...
	public function setShortDescription($v)
	{
		...
		if ($this->short_description !== $v) {
			...
			$this->modifiedColumns[] = ArRateCategoryPeer::SHORT_DESCRIPTION;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->short_description = ($row[$startcol + 2] !== null) ? (string) $row[$startcol + 2] : null;
			$this->resetModified();
			...
			return $startcol + 3; // 3 = ArRateCategoryPeer::NUM_COLUMNS - ArRateCategoryPeer::NUM_LAZY_LOAD_COLUMNS).
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = ArRateCategoryPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			if ($ret) {
				...
				$this->postSave($con);
				ArRateCategoryPeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(ArRateCategoryPeer::INTERNAL_NAME)) $criteria->add(ArRateCategoryPeer::INTERNAL_NAME, $this->internal_name);
		if ($this->isColumnModified(ArRateCategoryPeer::SHORT_DESCRIPTION)) $criteria->add(ArRateCategoryPeer::SHORT_DESCRIPTION, $this->short_description);
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of ArRateCategory (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     ArRateCategory Clone of current object.
	 * @throws     PropelException
	 */
	 ...
	 * Otherwise if this ArRateCategory has previously been saved, it will retrieve
	 * related ArOrganizationUnitHasStructures from storage. If this ArRateCategory is new, it will return
		...
		if ($this->collArOrganizationUnitHasStructures === null) {
			if ($this->isNew()) {
			   $this->collArOrganizationUnitHasStructures = array();
				...
				$criteria->add(ArOrganizationUnitHasStructurePeer::AR_RATE_CATEGORY_ID, $this->id);
	 ...
	 * Otherwise if this ArRateCategory is new, it will return
	 * an empty collection; or if this ArRateCategory has previously
	 * been saved, it will retrieve related ArOrganizationUnitHasStructures from storage.
	 *
	 * This method is protected by default in order to keep the public
	 * api reasonable.  You can provide public methods for those you
	 * actually need in ArRateCategory.
	 */
