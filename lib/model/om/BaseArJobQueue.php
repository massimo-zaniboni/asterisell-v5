...
/**
 * Base class that represents a row from the 'ar_job_queue' table.
 *
 * 
 *
...
abstract class BaseArJobQueue extends BaseObject  implements Persistent {
	...
	/**
	 * The value for the is_part_of field.
	 * @var        int
	 */
	...
	/**
	 * The value for the php_data_job_serialization field.
	 * @var        string
	 */
	...
	/**
	 * Initializes internal state of BaseArJobQueue object.
	 * @see        applyDefaults()
	 */
	...
	/**
	 * Get the [is_part_of] column value.
	 * 
	 * @return     int
	 */
	...
	/**
	 * Get the [php_data_job_serialization] column value.
	 * 
	 * @return     string
	 */
	 ...
	 * @return     ArJobQueue The current object (for fluent API support)
	 */
	...
	/**
	 * Set the value of [is_part_of] column.
	 * 
	 * @param      int $v new value
	 * @return     ArJobQueue The current object (for fluent API support)
	 */
	...
	/**
	 * Set the value of [php_data_job_serialization] column.
	 * 
	 * @param      string $v new value
	 * @return     ArJobQueue The current object (for fluent API support)
	 */
	public function setPhpDataJobSerialization($v)
	{
		...
		if ($this->php_data_job_serialization !== $v) {
			$this->php_data_job_serialization = $v;
			$this->modifiedColumns[] = ArJobQueuePeer::PHP_DATA_JOB_SERIALIZATION;
		}

		return $this;
	} // setPhpDataJobSerialization()
	...
	public function setInternalName($v)
	{
		...
		if ($this->internal_name !== $v) {
			...
			$this->modifiedColumns[] = ArJobQueuePeer::INTERNAL_NAME;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->is_part_of = ($row[$startcol + 1] !== null) ? (int) $row[$startcol + 1] : null;
			...
			$this->php_data_job_serialization = ($row[$startcol + 7] !== null) ? (string) $row[$startcol + 7] : null;
			$this->internal_name = ($row[$startcol + 8] !== null) ? (string) $row[$startcol + 8] : null;
			$this->resetModified();
			...
			return $startcol + 9; // 9 = ArJobQueuePeer::NUM_COLUMNS - ArJobQueuePeer::NUM_LAZY_LOAD_COLUMNS).
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = ArJobQueuePeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			if ($isInsert) {
				...
				// symfony_timestampable behavior
				if (!$this->isColumnModified(ArJobQueuePeer::CREATED_AT))
				{
			...
			if ($ret) {
				...
				$this->postSave($con);
				ArJobQueuePeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[8], $arr)) $this->setInternalName($arr[$keys[8]]);
	}
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(ArJobQueuePeer::INTERNAL_NAME)) $criteria->add(ArJobQueuePeer::INTERNAL_NAME, $this->internal_name);
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of ArJobQueue (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     ArJobQueue Clone of current object.
	 * @throws     PropelException
	 */
