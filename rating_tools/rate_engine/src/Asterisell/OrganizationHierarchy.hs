...
parseFileWithExtensions :: Bool -> String -> IO (Either (IsApplicationError, String) Info)
parseFileWithExtensions isDebugMode fileName
       ...
       case eitherResult r of
         ...
         Right (info, _, errors, _)
           -> case Set.null errors of
                ...
                False
                  -> return $ Left $ (False
                                     , "There are extensions with problems. " ++ (L.concatMap (\t -> t ++ "\n") (Set.toList errors))
                                     )
...
extensionCodes_extractFromUserSpecification :: String -> [ExtensionCode]
extensionCodes_extractFromUserSpecification userCodes
  = extractResult $ L.foldl' proc ([], [], False, True)  userCodes
          ...
          = case (structure_extensionCodes dataInfo) of
              ...
              Just codes
                -> let -- | Extract the codes separated from ",", and manage quoted value: "\,"
                       ...
                       listOfCodes = extensionCodes_extractFromUserSpecification $ T.unpack codes
