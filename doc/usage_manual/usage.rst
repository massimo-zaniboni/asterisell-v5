...
Application Recurring Concepts
------------------------------

Customers are defined using nested trees, for example "Acme Organization/Accounting Office/John Smith".

Also CDR rating rules are defined using nested trees, for example "acme-vendor-telecom/outgoing/emergency-number".

Whenever possible a complete history of the past is retained. This is
the case for rates, services, customer price categories, customer organization structure.

So the common workflow is not modifying information in-place,
...
in-place changes. But there are many advantages:

-  changes of rates and services can be planned in the future;
-  CDRs can be re-rated in the past, applying the old rate
   specifications;
-  history of applied rates, and services is retained;

Asterisell uses a conservative approach about CDR rating.
...
calculating the wrong cost/income.

Every reported error contains:

-  the error level (info, warning, error, critical)
-  the description
-  the effect on the system
-  the suggested solution for fixing it

Configuration of the CDR Rating Process
---------------------------------------

Overview
........

The configuration is an iterative process:

-  initial configuration attempt
-  read Asterisell error messages
-  improve configuration, until there are no more error messages

Asterisell refuses to process CDRs if configurations are not 100%
...
Asterisell, can guide you to the resolution of problems.

Paid Support for Initial Configuration
......................................

Because initial configuration of Asterisell is the more complex task to
do, you can describe your needs to support_ and the
Asterisell support can customize and configure Asterisell for you. Then
the maintenance of a properly configured Asterisell instance is easier.
...
channel, used for retrieving CDRs.

Note that a CDR provider is not a Vendor. A Vendor is a fiscal entity
that routes CDRs, and that can requires payments for this service.

In many configurations there is a one to one match between a Vendor and
...
many Vendors, or a Vendor uses more than one CDR provider.

Communication Channels types are created in
``Params -> Communication Channels``. A cummunication channel type is
something like:
...
A Communication Channel is a value inside CDR identifying the channel
used for routing the call.
...
Vendors or Communication Channel Types to a CDR.
...
Manual Importing of CDRs files
..............................

The source CDRs files must be put inside the directory
``data_files/messages/input``. They must have a name with the previously
...
Automatic Importing of CDRs files
.................................
...
Workflow of CDRs Processing
~~~~~~~~~~~~~~~~~~~~~~~~~~~

::

    [VoIP Servers] ---put CDRs on---> [local collector table]
    ... ---Asterisell exports to---> [CSV files]
    ... ---rated to---> [Rated CDR records]

The ``CSV Files`` are archived in the file-system, and directly
referenced and managed from Asterisell. The Asterisell backup procedures
can take care also of these files.

``Rated CDR Records`` are compressed, and indexed. They are managed from
Asterisell application, and from Asterisell backup procedures.
...
-  ask for support_, specifying how the CDRs must be processed;
...
rarely the main rating plan logic.
...
If you have few time, you can describe your rating plan to Asterisell
assistance, and we will specify it for you.

Call Reporting Mode
-------------------

In Call Reporting Mode, Asterisell is not used for billing the calls.
Calls have only a cost.

The user must specify only the ``main-income-rate``. It is called
``income`` but it is rating the costs of calls. The ``income`` rate is
preferred because it is more powerful and it can contains also bundle
rate parameters.

The resulting CDRs will have an income and cost equals to the result of
the rate.

Automatic Rerating of Calls
---------------------------
...
Customization of Messages to Customers
--------------------------------------

Type of Messages
................

Customers can receive from Asterisell:
...
Reports and invoices are attached to an email. The email can be
...
that is good for all different usage scenario.
...
You can define a normal administrator with these roles:

-  ``Notified for warning errors``
-  ``Notified for errors``
-  ``Notified for critical errors``

He will be advised of all type of errros.

Emergency Email
...............

You can define another account, with a distinct email, and with only
this role:
